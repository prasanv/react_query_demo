...
export default function infiniteQueries() {
  ...
  const result = useInfiniteQuery(["infiniteQueries"], fetchData, {
    // NOTE: useInfiniteQuery - getNextPageParam, fetchNextPage, hasNextPage
    // `getNextPageParam` - `_lastPage` parameter is mandatory but not sure why
  ...
  return (
    <div>
      ...
      <Button onClick={() => fetchNextPage()} disabled={!result.hasNextPage}>
        Load Content
      </Button>
    </div>
  );
